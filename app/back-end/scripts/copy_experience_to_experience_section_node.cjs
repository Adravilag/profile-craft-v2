#!/usr/bin/env node
/* Copy Experience collection to experience-section with localization for position and description */
...
async function main() {
  ...
  const sourceCollName = 'experiences';
  const targetCollName = 'experience-section';
  ...
  try {
    ...
    while (await cursor.hasNext()) {
      ...
      // Localize position and description
      if (typeof newDoc.position === 'string') newDoc.position = normalize(newDoc.position);
