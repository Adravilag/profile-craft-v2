...
const ProjectsCarousel: React.FC<Props> = ({ projects: initialProjects }) => {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      const msg = err && (err as any).message ? (err as any).message : 'Error al obtener proyectos';
      setError(msg);
    } finally {
      if (!mountedRef.current) return;
      setLoading(false);
      ...
      >
        {projects.map((p, i) => (
          <button
            ...
            onClick={() => goTo(i)}
            aria-label={`${i === index ? 'Proyecto actual, ' : ''}Ir al proyecto ${i + 1}${p.title ? `: ${p.title}` : ''}`}
        ...
        return (
          <div className={styles.contentWrapper}>
            <div className={styles.errorBanner} role="status" aria-live="polite">
              <strong>Problema cargando proyectos:</strong> {error}
              <button
        ...
        <div className={styles.emptyState} role="alert" aria-live="assertive">
          <div className={styles.emptyCard}>
            ...
            <div className={styles.emptyText}>
              <div className={styles.emptyTitle}>No pudimos cargar los proyectos</div>
      ...
      return (
        <div className={styles.emptyState} role="status" aria-live="polite">
          <div className={styles.emptyCard}>
            ...
            <div className={styles.emptyText}>
              <div className={styles.emptyTitle}>No hay proyectos para mostrar</div>
              <div className={styles.emptySubtitle}>
                Aún no hay proyectos públicos en este perfil. Puedes añadir proyectos desde el panel
                de administración o comprobar tus filtros.
              </div>
              <div className={styles.emptyActions}>
            ...
            <div className={styles.detailsPane} aria-live="polite">
                ...
                {current.status &&
                  (String(current.status) === 'completed' ? (
                    <span
                      ...
                      aria-label="Proyecto completado"
                    >
                      ✔ COMPLETADO
                    </span>
                  ) : (
              ...
              <div className={styles.metaRow}>
                {current.published_at && (
                  <div
                    ...
                    aria-label={`Publicado el ${new Date(current.published_at).toLocaleDateString('es-ES')}`}
                  >
              ...
              <div className={styles.titleRow}>
                ...
                {typeof current.views === 'number' && (
                  <div className={styles.viewsInline} aria-label={`Vistas ${current.views}`}>
                    <svg
      ...
      >
        <span className={styles.srOnly} aria-live="polite">
          {loading
            ? 'Cargando proyectos'
            : length > 0
              ? `Proyecto ${index + 1} de ${length}: ${current?.title}`
              : 'No hay proyectos'}
        </span>
